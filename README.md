...
> 3. **盘口数据不可回补** —— 交易所不提供历史 bid/ask，采样器停了就永久丢失。
>    但注意一条**例外经验**：若另一台机器（乙侧）当时在采，**首档点差**仍可从它那里补
>    （5 档深度补不了）。实测：09-20 那次 12h16m 停摆，首档靠乙侧 W2 样本补回 **99.7%**
>    （`docs/51` §6.4），而 5 档至今是空的 —— **"不可回补"说的是交易所这条路，
>    不等于"没有别的来源"，但也不能因此指望每次都有备份。**
