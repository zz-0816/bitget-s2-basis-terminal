...
async function boot() {
  ...
  // ⚠️ 给"自动化验收"一个**明确的落定信号**（`window.__settled`）。
  //    为什么需要它（2026-09-25 实测踩到）：真浏览器验收原本只等"页面没有可见骨架屏"，
  //    而 `sweepSkeletons` 兜底会把骨架屏换成文字 —— 于是那个条件**在真实数据到达之前
  //    就被满足了**，截图拍到的是三张空卡，验收于是报"判据实测 0"（而接口其实好的）。
  //    正确的判据是"首批数据已经落定（**成功或失败都算落定**）"：
  //    真失败时卡片的 data-state 会空着，验收就该在**卡片检查**上报错，
  //    而不是在"拍了张空页"上报一个看不懂的错。
  window.__settled = 1;
